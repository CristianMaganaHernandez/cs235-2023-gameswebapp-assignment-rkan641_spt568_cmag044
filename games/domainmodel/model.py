...
class Game:
    ...
    def add_review(self, review):
        if not isinstance(review, Review):
            raise ValueError("Review must be an instance of Review class")
...
class User:
    ...
    def __lt__(self, other):
        if not isinstance(other, self.__class__):
            return False
        return self.__username.lower() < other.username.lower()

    @property
    def wishlist(self):
        return self._wishlist

    def add_to_wishlist(self, game_id):
        if game_id not in self._wishlist:
            self._wishlist.append(game_id)

    def remove_from_wishlist(self, game_id):
        if game_id in self._wishlist:
            self._wishlist.remove(game_id)
    ...
    def is_anonymous(self):
        # This should return False, as real users are not anonymous
        return False

    def get_id(self):
        # Return the unique identifier for this user, which is their username
        return self.__username
...
def add_review(comment: str, rating: int, game: Game, user: User):
    review = Review(user=user, game=game, rating=rating, comment=comment)
